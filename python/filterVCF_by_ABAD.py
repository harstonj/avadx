...
with (lambda f: (gzip.open if f.endswith('.gz') else open)(f, 'rt'))(vcf_input) as vcf_handle, gzip.open(vcf_output, 'wt') as fw, open(vcf_output + '.log', 'w') as wlog:
    ...
    for line in vcf_handle:
        if line[0] != '#':
            ...
            dp = [-1 if x[DP_idx] == '.' else int(x[DP_idx]) for x in idv_list] if DP_idx >= 0 else []  # -1 means no call

            GQ_idx = fm_list.index('GQ') if has_GQ else -1
            gq = [-1 if x[GQ_idx] == '.' else int(x[GQ_idx]) for x in idv_list] if GQ_idx >= 0 else []   # -1 means no call
            ...
            # Calculate AB
            ad_length = len(ad)
            if ad_length != 0:  # Calculate AB from AD
                idx = 0
                for a in [x.split(',') for x in ad]:  # a: ['0', '37']
                    GT = gt[idx]
                    if '.' in a or './.' in a:
                        ab += [-1]  # no call
                    else:
                        ...
                        if len(a) > 2:  # multi-allelic loci
                            ...
                            if GT == '0/1':
                                ...
                                ab += [99]  # homozygous calls
                        else:
                            ab += [-3]  # sum(AD) == 0 or others
                    idx += 1

                # Count bad genotypes
                n_bad_GTs, total_GTs = 0, len(gt)
                cutoff = ceil(total_GTs * cutoff_MR)
                keep = True
                for i in range(0, total_GTs):
                    check_AB, check_DP, check_GQ = ab[i], dp[i], gq[i]
                ...
                if keep:
                    fw.write(line)
                    if line_cnt % line_buffer == 0:
                        fw.flush()
                else:
                    wlog.write('\t'.join(row[0:5]) + '\tHigh_Missing_Rate\n')
            elif ad_length == 0:  # No AB and no AD available.
